...
class AssetMigrationLocalDataSourceImpl
  ...
  @override
  Future<AssetMigrationModel> createAssetConsumable(
    ...
    final response = await db.transaction((txn) async {
      final checkAssetLocation = await txn.query(
        'SELECT COUNT(id) FROM t_assets WHERE asset_model_id = ? AND location_id = ?',
        [params.assetModelId, params.locationDetailId],
      );

      int idAsset;

      if (checkAssetLocation.first.fields['COUNT(id)'] == 0) {
        // Insert
        ...
        // Update Quantity
        idAsset = checkAssetLocation.first.fields['COUNT(id)'] as int;
  ...
  Future<AssetMigrationModel> createNewAsset(
    ...
    try {
      ...
      final resposne = await db.transaction((txn) async {
      ...
      return AssetMigrationModel.fromMap(resposne!);
    } catch (e) {
      print(e);
      throw e;
    }
  ...
  @override
  Future<List<AssetMigrationModel>> findAllAsset() async {
    final db = await _database.connection;
    ...
    if (response.firstOrNull == null) {
      throw NotFoundException(
        message: 'Asset still empty, please create first',
      );
    }

    return response.map((e) => AssetMigrationModel.fromMap(e.fields)).toList();
  }
